...
<script>
    function checkout(loggedInUser) {
        const loggedInCustomer = @auth('customer') {!! json_encode(Auth::guard('customer')->user()->only(['id', 'name', 'email'])) !!} @else null @endauth;

        return {
            cart: {},
            customer: {
                name: loggedInCustomer ? loggedInCustomer.name : '',
            ...
            placeOrder() {
                ...
                fetch('/api/orders', {
                    ...
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    body: JSON.stringify({
                        cart: Object.values(this.cart), // Convert cart object to an array
